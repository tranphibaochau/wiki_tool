...
def get_random_wikipedia_article():
    ...
    while failed:
        ...
        failed = False
        try:
            req = urllib2.Request('https://en.wikipedia.org/wiki/Wikipedia:Administrators%27_noticeboard/Incidents',
                                  None, { 'User-Agent' : 'x'})
        ...
        try:
            ...
            all = re.sub(r'\&lt;.*?&gt;', '', all)
            all = re.sub(r'bgcolor=.{8}' ,'',all)
            # all = re.sub(r'\^((http[s]?|ftp):\/)?\/?([^:\/\s]+)((\/\w+)*\/)([\w\-\.]+[^#?\s]+)(.*)?(#[\w\-]+)?$' ,'',all)
            all = re.sub(r'\||\=' ,'',all)
            print (all)
        except:
...
if __name__ == '__main__':
    ...
    for i in range(0, len(articles)):
        url = urllib.unquote(articlenames[i]).decode('utf8')
        url = re.sub('[^A-Za-z0-9\_]+', '', url)
        if len(url) > 20:
            url=url[:20]
        url = url.lower()
        output_dir="C:\\Users\\seban\\Downloads"
        file_loc = "C:\\Users\\seban\\Downloads\\" + url +".txt"
        #print file_loc
